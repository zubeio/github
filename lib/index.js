            ...
            var req = require(reqModule).request(options, function(res) {
                ...
                res.on("end", function() {
                    if (res.statusCode >= 400 && res.statusCode < 600 || res.statusCode < 10) {
                        ...
                        if (options.headers.Authorization && options.headers.Authorization.indexOf('token v1.') === 0) {
                            http_err.request_options = JSON.parse(JSON.stringify(options));
                            http_err.request_options.headers.Authorization = 'token v1.' + http_err.request_options.headers.Authorization.split('token v1.')[1].slice(0,8) + '*****';
                        }
