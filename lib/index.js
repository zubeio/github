...
(function() {
        ...
        function prepareApi(struct, baseType) {
            ...
            Object.keys(struct).forEach(function(routePart) {
                ...
                if (block.url && block.params) {
                    ...
                    self[section][funcName] = function(originalMsg, callback) {
    ...
    this.httpSend = function(msg, block, callback) {
        ...
        if (method !== 'get') headers["content-length"] = "0"; // zube addition
        if (hasBody) {
            if (format == "json") {
            ...
            switch (this.auth.type) {
                case "oauth":
                    if (this.auth.token) {
                        ...
                        // marketplace deprecated this, I think. Use basic auth instead
                        // path += (path.indexOf("?") === -1 ? "?" : "&") +
                        //     "client_id=" + encodeURIComponent(this.auth.key) +
                        //     "&client_secret=" + encodeURIComponent(this.auth.secret);
                        basic = new Buffer(this.auth.key + ":" + this.auth.secret, "ascii").toString("base64");
        ...
        function httpSendRequest() {
            ...
            var req = require(reqModule).request(options, function(res) {
                ...
                res.on("end", function() {
                    if (res.statusCode >= 400 && res.statusCode < 600 || res.statusCode < 10) {
                        var httpErr = new error.HttpError(data, res.statusCode, res.headers);
                        var request_options = JSON.parse(JSON.stringify(options));
                        var is_obscured = false;
                        // application oauth
                        var client_secret_match = request_options.path.match(/client_secret=[a-z0-9]{40}/);
                        if (client_secret_match) {
                            request_options.path = request_options.path.replace(client_secret_match[0], client_secret_match[0].slice(0,22) + '******');
                            is_obscured = true;
                        }
                        // integration and installation
                        if (request_options.headers.Authorization) {
                            request_options.headers.Authorization = request_options.headers.Authorization.slice(0, 16) + '******';
                            is_obscured = true;
                        }
                        // user oauth
                        var access_token_match = request_options.path.match(/access_token=[a-z0-9]{40}/);
                        if (access_token_match) {
                            request_options.path = request_options.path.replace(access_token_match[0], access_token_match[0].slice(0,21) + '******');
                            is_obscured = true;
                        }
                        if (is_obscured) httpErr.request_options = request_options;
                        callCallback(httpErr);
                    } else {
                        res.data = data;
            ...
            req.on("timeout", function() {
                ...
                req.abort();
                var timeoutErr = new error.GatewayTimeout();
                var request_options = JSON.parse(JSON.stringify(options));
                var is_obscured = false;
                // application oauth
                var client_secret_match = request_options.path.match(/client_secret=[a-z0-9]{40}/);
                ...
                if (is_obscured) timeoutErr.request_options = request_options;
                callCallback(timeoutErr);
            });
