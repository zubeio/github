...
(function() {
    ...
    this.httpSend = function(msg, block, callback) {
        ...
        function httpSendRequest() {
            ...
            if (options.method === 'get') delete options.headers['content-length']; // so we don't send "content-length": "0" for GET requests
            var req = require(reqModule).request(options, function(res) {
                ...
                res.on("end", function() {
                    if (res.statusCode >= 400 && res.statusCode < 600 || res.statusCode < 10) {
                        ...
                        var access_token_match = options.path.match(/access_token=[a-z0-9]{40}/);
                        if (access_token_match) {
                            http_err.request_options = JSON.parse(JSON.stringify(options));
                            http_err.request_options.path = http_err.request_options.path.replace(access_token_match[0], access_token_match[0].slice(0,21) + '******');
                        }
